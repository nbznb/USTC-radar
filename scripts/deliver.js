...
async function sendEmail(text, apiKey, toEmail) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'USTC Radar <digest@resend.dev>',
      to: [toEmail],
      subject: `USTC Radar - ${new Date().toLocaleDateString('zh-CN', {
  ...
  try {
    switch (delivery.method) {
      case 'telegram': {
        ...
        console.log(JSON.stringify({ status: 'ok', method: 'telegram', message: 'USTC Radar sent to Telegram' }));
        break;
      }
      ...
      case 'email': {
        ...
        console.log(JSON.stringify({ status: 'ok', method: 'email', message: `USTC Radar sent to ${toEmail}` }));
        break;
      }
